...
def output_png(folder, images, labels, prefix):
    if not os.path.exists(os.path.join(folder, prefix)):
        os.mkdir(os.path.join(folder, prefix))

    for i in range(len(images)):
        out = os.path.join(folder, prefix, "%06d-num%d.png"%(i,labels[i]))
        imageio.imsave(out, np.array(images[i]).reshape(28,28))
...
def process(folder, imgf, labelf, prefix, n):
    ...
    output_png(folder, images, labels, prefix)
    output_csv(folder, images, labels, prefix)
    
def read_images_folder(path):
    labels = []
    imgs = []

    for filename in os.listdir(path):
        if filename.endswith(".png"): 
            label = filename.split('-')[1].replace('num', '').replace('.png', '')
